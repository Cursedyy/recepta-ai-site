  ...
  for (let i = 0; i < args.length; i++) {
    switch (args[i]) {
      ...
      case "--max-total":
        ...
        break;
      case "--max-critical":
        config.maxCritical = parseInt(args[++i]) || 0;
        break;
      case "--base-url":
  ...
  try {
    ...
    const overflowElements = await page.evaluate((vpWidth) => {
      ...
      for (const el of allElements) {
        ...
        // Off-canvas: painel que comeca inteiro fora da viewport (drawer mobile
        // fechado com translateX(100%)) nao vaza nada — o scroll horizontal real
        // ja e coberto por hasOverflow no documento.
        if (rect.left >= vpWidth) continue;
    ...
    return {
      ...
      passed:
        overflowCount <= config.maxOverflow &&
        touchCount <= config.maxTouchSmall,
    };
  } catch (error) {
    return {
      page: pageDef.name,
...
async function main() {
  ...
  console.log(
    `Thresholds: overflow=${config.maxOverflow}, touch=${config.maxTouchSmall}, total=${config.maxTotal}`,
  );
  console.log("");
  ...
  for (const pageDef of PAGES) {
    ...
    for (const viewport of VIEWPORTS) {
      ...
      if (result.error) {
        ...
        console.log(
          `  ⚠️  ${viewport.name}: FAIL (${result.overflow} overflow, ${result.touchTargets} touch)`,
        );
        if (pageDef.critical) {
  ...
  if (overallPassed) {
    ...
    console.log("\n❌ OVERALL: FAIL");
    console.log(
      `   Critical issues (${criticalIssues}) exceeded threshold (${config.maxCritical})`,
    );
    console.log(
      `   Total issues (${totalIssues}) exceeded threshold (${config.maxTotal})`,
    );
    process.exit(1);
  }
}
